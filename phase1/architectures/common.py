...
def batchNormalization(x, is_training= True, decay= 0.9, epsilon= 0.001):
    ...
    if is_training:
      mean, variance = tf.compat.v1.nn.moments(x, axis)
      ...
      return tf.compat.v1.nn.batch_normalization(x, moving_mean, moving_variance, beta, gamma, epsilon)
...
def flatten(x):
    ...
    for i in xrange(1,len(shape)):
      dim*=shape[i]
    return tf.compat.v1.reshape(x, [-1, dim])

def treshold(x, treshold):
    return tf.compat.v1.cast(x > treshold, x.dtype) * x

def fullyConnected(x, num_units_out, wd= 0.0, weight_initializer= None, bias_initializer= None):
    num_units_in = x.get_shape()[1]

    stddev = 1./tf.sqrt(tf.cast(num_units_out, tf.compat.v1.float32))
    if weight_initializer is None:
      weight_initializer = tf.compat.v1.random_uniform_initializer(minval= -stddev, maxval= stddev, dtype= tf.compat.v1.float32)
    if bias_initializer is None:
      bias_initializer = tf.compat.v1.random_uniform_initializer(minval= -stddev, maxval= stddev, dtype= tf.compat.v1.float32) 

    weights = _get_variable('weights',
                            [num_units_in, num_units_out], weight_initializer, tf.compat.v1.contrib.layers.l2_regularizer(wd))
...
def spatialConvolution(x, ksize, stride, filters_out, wd= 0.0, weight_initializer= None, bias_initializer= None):
    ...
    weights = _get_variable('weights',
                            shape, weight_initializer, tf.compat.v1.contrib.layers.l2_regularizer(wd))

    conv = tf.compat.v1.nn.conv2d(x, weights, [1, stride, stride, 1], padding= 'SAME')
    biases = _get_variable('biases', [filters_out],  bias_initializer)
            
    return tf.compat.v1.nn.bias_add(conv, biases)
    
def maxPool(x, ksize, stride):
    return tf.compat.v1.nn.max_pool(x,
                          ksize=[1, ksize, ksize, 1],
                          strides=[1, stride, stride, 1],
                          padding='SAME')

def avgPool(x, ksize, stride, padding='SAME'):
    return tf.compat.v1.nn.avg_pool(x,
                          ksize=[1, ksize, ksize, 1],
                          strides=[1, stride, stride, 1],
                          padding= padding)
...
def resnetBlock(x, bottleneck, block_filters_internal, block_stride, wd= 0.0, is_training= True):
    ...
    return tf.compat.v1.nn.relu(x + shortcut)
