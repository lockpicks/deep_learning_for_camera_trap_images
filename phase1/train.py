...
def loss(logits, labels):
  ...
  cross_entropy = tf.compat.v1.nn.sparse_softmax_cross_entropy_with_logits(labels= labels, logits= logits, name= 'cross_entropy_per_example')
  cross_entropy_mean = tf.compat.v1.reduce_mean(cross_entropy, name= 'cross_entropy')

  #Add a Tensorboard summary
  tf.compat.v1.summary.scalar('Cross Entropy Loss', cross_entropy_mean)
...
def train(args):
  ...
  with tf.compat.v1.Graph().as_default(), tf.compat.v1.device('/cpu:0'):

    # Read data from disk
    images, labels = data_loader.read_inputs(True, args)

    epoch_number = tf.compat.v1.get_variable('epoch_number', [], dtype= tf.int32, initializer= tf.constant_initializer(0), trainable= False)

    # Decay the learning rate
    lr = tf.compat.v1.train.piecewise_constant(epoch_number, args.LR_steps, args.LR_values, name= 'LearningRate')
    # Weight Decay policy
    wd = tf.compat.v1.train.piecewise_constant(epoch_number, args.WD_steps, args.WD_values, name= 'WeightDecay')
      ...
      for i in xrange(args.num_gpus):
        with tf.compat.v1.device('/gpu:%d' % i):
          with tf.compat.v1.name_scope('Tower_%d' % i) as scope:
            ...
            # Top-1 accuracy
            top1acc = tf.compat.v1.reduce_mean(
                tf.compat.v1.cast(tf.compat.v1.nn.in_top_k(logits, labels, 1), tf.compat.v1.float32))
            # Top-5 accuracy
            topnacc = tf.compat.v1.reduce_mean(
                tf.compat.v1.cast(tf.compat.v1.nn.in_top_k(logits, labels, args.top_n), tf.compat.v1.float32))
            ...
            reg_loss = tf.compat.v1.add_n(regularization_losses)

            #Add a tensorboard summary
            tf.compat.v1.summary.scalar('Regularization Loss', reg_loss)

            # Compute the total loss (cross entropy loss + regularization loss)
            total_loss = tf.compat.v1.add(cross_entropy_mean, reg_loss)
            ...
            tf.compat.v1.summary.scalar('Top-1 Accuracy', top1acc)
            tf.compat.v1.summary.scalar('Top-'+str(args.top_n)+' Accuracy', topnacc)
            ...
            batchnorm_updates = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS, scope)

            # Calculate the gradients for the batch of data on this CIFAR tower.
            if args.transfer_mode[0]== 3:
              grads = opt.compute_gradients(total_loss, var_list= tf.compat.v1.get_collection(tf.GraphKeys.VARIABLES, scope='output'))
    ...
    summaries.append(tf.compat.v1.summary.scalar('weight_decay', wd))

    # Group all updates to into a single train op.
    #with tf.control_dependencies(bn_update_ops):
    # Setup the train operation
    if args.transfer_mode[0]==3:
      train_op = tf.compat.v1.cond(tf.compat.v1.less(epoch_number,args.transfer_mode[1]),
              lambda: tf.compat.v1.group(opt.apply_gradients(grads),*batchnorm_updates), lambda: tf.compat.v1.group(opt.apply_gradients(auxgrads),*batchnorm_updates))
    elif args.transfer_mode[0]==1:
      train_op = opt.apply_gradients(grads)
    else:
      batchnorm_updates_op = tf.compat.v1.group(*batchnorm_updates)
      train_op = tf.compat.v1.group(opt.apply_gradients(grads), batchnorm_updates_op)
    ...
    if args.retrain_from is not None:
      if args.transfer_mode[0]==0:
        pretrained_loader = tf.compat.v1.train.Saver()
      else:
        pretrained_loader = tf.compat.v1.train.Saver(var_list= exclude())
    ...
    if args.retrain_from is not None:
      ckpt = tf.compat.v1.train.get_checkpoint_state(args.retrain_from)
